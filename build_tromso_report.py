...
DATA_DIR = ROOT / "Eliteserien"
OUT_DIR = DATA_DIR / "output" / "pdf"
ASSET_DIR = OUT_DIR / "tromso_shooting_mplsoccer_assets"
TMP_DIR = DATA_DIR / "tmp" / "pdfs"
PDF_PATH = OUT_DIR / "Tromso_IL_opposition_report.pdf"
...
def footer(draw, source="StatsBomb event data | 2026 Eliteserien"):
...
def save_shotmap(path, shots, title, subtitle):
    ...
    fig.text(0.05, 0.04, "StatsBomb event data | 2026 Eliteserien", color=MUTED, fontsize=10)
...
def save_match_flow(path, match, all_shots):
    ...
    fig.text(0.05, 0.845, f"Tromsø {result_string(match)} {opponent_name}", color=WHITE, fontsize=29, fontweight="bold")
...
def save_pass_network(path, match, events):
    ...
    plot_page_frame(
        ...
        f"Tromsø {result_string(match)} {opponent(match)}",
...
def save_pass_selection(path, match, events, kind):
    ...
    plot_page_frame(
        fig, kicker, f"Tromsø {result_string(match)} {opponent(match)}",
...
def save_xg_channels(path, match, all_shots):
    ...
    plot_page_frame(fig, "SHOOTING | xG BY ATTACKING CHANNEL", f"Tromsø {result_string(match)} {opponent(match)}",
...
def save_zone_origin_passes(path, match, events, zone):
    ...
    plot_page_frame(fig, kicker, f"Tromsø {result_string(match)} {opponent(match)}", match_subtitle(match, subtitle))
    fig.text(0.84, 0.68, str(len(selected)), color=color, fontsize=30, fontweight="bold")
...
def save_pressing(path, match, events):
    ...
    plot_page_frame(fig, "OFF-BALL | PRESSING MAP", f"Tromsø {result_string(match)} {opponent(match)}",
...
def save_progression(path, match, events):
    ...
    plot_page_frame(fig, "ON-BALL | BALL PROGRESSION", f"Tromsø {result_string(match)} {opponent(match)}",
...
def save_defensive(path, match, events):
    ...
    plot_page_frame(fig, "OFF-BALL | DEFENSIVE ACTIONS", f"Tromsø {result_string(match)} {opponent(match)}",
...
def save_build_up(path, match, events):
    ...
    plot_page_frame(fig, "ON-BALL | GOAL-KICK BUILD-UP", f"Tromsø {result_string(match)} {opponent(match)}",
...
def save_transitions(path, match, events):
    ...
    plot_page_frame(fig, "ON-BALL | ATTACKING TRANSITIONS", f"Tromsø {result_string(match)} {opponent(match)}",
...
def save_pressure_turnovers(path, match, events):
    ...
    plot_page_frame(fig, "ON-BALL | TURNOVERS UNDER PRESSURE", f"Tromsø {result_string(match)} {opponent(match)}",
...
def save_chance_network(path, match, events):
    ...
    plot_page_frame(fig, "ON-BALL | CHANCE-CREATION NETWORK", f"Tromsø {result_string(match)} {opponent(match)}",
...
def save_possession_funnel(path, match, events):
    ...
    plot_page_frame(fig, "ON-BALL | POSSESSION OUTCOME FUNNEL", f"Tromsø {result_string(match)} {opponent(match)}",
                    match_subtitle(match, "unique Tromsø possessions reaching each stage"))
    fig.savefig(path, facecolor=NAVY, dpi=100); plt.close(fig)
...
def save_set_piece_comparison(path, recent, events_by_id, mode):
    config = {
        "attacking_corners": ("SET PIECES | ATTACKING CORNERS", "Tromsø corner deliveries and resulting shots", "From Corner", TEAM, False),
        "attacking_free_kicks": ("SET PIECES | ATTACKING FREE KICKS", "Tromsø free-kick deliveries and resulting shots", "From Free Kick", TEAM, False),
        "defending_corners": ("SET PIECES | DEFENDING CORNERS", "Opponent corner deliveries into Tromsø's defensive area", "From Corner", "OPP", True),
    }
...
def save_league_zscores(path, metrics):
    ...
    plot_page_frame(fig, "BENCHMARKING | LEAGUE Z-SCORES", "Tromsø attacking and possession profile",
                    "2026 Eliteserien | positive = above league average")
    fig.savefig(path, facecolor=NAVY, dpi=100); plt.close(fig)
...
def save_league_beeswarm(path, metrics):
    ...
    for spine in ax.spines.values(): spine.set_visible(False)
    plot_page_frame(fig, "BENCHMARKING | LEAGUE DISTRIBUTIONS", "Where Tromsø sit within Eliteserien",
                    "pink = Tromsø | cyan = other teams | right label = league rank")
    fig.savefig(path, facecolor=NAVY, dpi=100); plt.close(fig)
...
def save_style_radar(path, metrics):
    ...
    plot_page_frame(fig, "BENCHMARKING | STYLE RADAR", "Tromsø league-percentile profile",
                    "2026 Eliteserien | outer ring = league-leading")
    fig.savefig(path, facecolor=NAVY, dpi=100); plt.close(fig)
...
def save_possession_distribution(path, recent, events_by_id):
    ...
    plot_page_frame(fig, "POSSESSION STYLE | DURATION DISTRIBUTION", "How long Tromsø keep each possession",
...
def save_pass_distribution(path, recent, events_by_id):
    ...
    plot_page_frame(fig, "PASSING STYLE | LENGTH DISTRIBUTION", "Short circulation versus direct passing",
                    "all Tromsø passes in the last three matches")
    fig.savefig(path, facecolor=NAVY, dpi=100); plt.close(fig)
...
def save_pass_direction(path, recent, events_by_id):
    ...
    plot_page_frame(fig, "PASSING STYLE | DIRECTION ROSE", "Directional tendencies of Tromsø passing",
...
def tromso_opponent_events(matches, events_by_id):
    rows = []
    for match in matches:
...
def save_vulnerability_sources(path, matches, events_by_id):
    ...
    for match, e in tromso_opponent_events(matches, events_by_id):
    ...
    for i, k in enumerate(order):
        ax.text(vals[i]+.25, i, f"{vals[i]:.2f} xGA | {agg[k][1]} shots | {agg[k][2]} goals",
                color=WHITE, fontsize=11, fontweight="bold", va="center")
    plot_page_frame(fig, "HOW TO HURT TROMSØ | CHANCE SOURCE", "Opposition xG by attacking source",
                    "2026 season | restarts are separated from open play")
    fig.savefig(path, facecolor=NAVY, dpi=100); plt.close(fig)


def save_conceded_danger_map(path, matches, events_by_id):
    fig, ax, pitch = combined_pitch_figure("Where Tromsø concede chances", "Season opposition shots | marker size = xG | goals outlined", "HOW TO HURT TROMSØ | SEASON MAP")
    shots = []
    for match, e in tromso_opponent_events(matches, events_by_id):
...
def save_opponent_box_entries(path, matches, events_by_id):
    fig, ax, pitch = combined_pitch_figure("How opponents enter Tromsø's penalty area", "Season completed passes entering the box", "HOW TO HURT TROMSØ | SEASON MAP")
...
def save_set_piece_matrix(path, matches, events_by_id):
    ...
    for pattern in types:
        shots = [e for _, e in tromso_opponent_events(matches, events_by_id)
    ...
    plot_page_frame(fig, "HOW TO HURT TROMSØ | SET-PIECE MATRIX", "Restart vulnerability",
                    "season shots, xGA and goals conceded by restart type")
    fig.savefig(path, facecolor=NAVY, dpi=100); plt.close(fig)


def save_second_phase_map(path, matches, events_by_id):
    fig, ax, pitch = combined_pitch_figure("Second-phase locations after restarts", "First opponent on-ball action after an attacking corner/free-kick delivery", "HOW TO HURT TROMSØ | SEASON MAP")
...
def save_opponent_key_passes(path, matches, events_by_id):
    fig, ax, pitch = combined_pitch_figure("Opposition key-pass origins", "Season shot assists linked to resulting shot locations", "HOW TO HURT TROMSØ | SEASON MAP")
    n = 0; xga = 0
    for match in matches:
        if TEAM not in {team_name(match, "home"), team_name(match, "away")}: continue
        ev = events_by_id.get(match["match_id"], []); opp = opponent(match)
...
def save_line_breaks(path, matches, events_by_id):
    fig, ax, pitch = combined_pitch_figure("Passes received behind Tromsø's block", "Proxy line breaks: completed opponent passes crossing x=80 or ending behind x=100", "HOW TO HURT TROMSØ | SEASON MAP")
    counts = defaultdict(int)
    for match, e in tromso_opponent_events(matches, events_by_id):
...
def save_far_post_entries(path, matches, events_by_id):
    fig, ax, pitch = combined_pitch_figure("Weak-side and far-post entries", "Completed opponent deliveries from one flank to the opposite half of the box", "HOW TO HURT TROMSØ | SEASON MAP")
...
def save_press_targets(path, recent, events_by_id):
    ...
    plot_page_frame(fig, "HOW TO HURT TROMSØ | PRESSING TARGETS", "Players most vulnerable under pressure",
...
def save_match_plan(path):
    ...
    header(d, "HOW TO HURT TROMSØ", "FC Hradec Králové match plan", "Evidence-led attacking priorities")
    items = [
        ("01", "WIN RESTARTS", "Corners, free kicks and throw-ins are repeatable routes into Tromsø's defensive box."),
    ...
    footer(d, "StatsBomb event data | 2026 Eliteserien | opposition vulnerability synthesis")
    img.save(path, quality=95)
...
def save_formation_analysis(path, matches, events_by_id):
    ...
    plot_page_frame(fig, "REFERENCE VISUAL | FORMATION & POSITIONAL ANALYSIS", "Tromsø shape and positional tendencies",
...
def save_progressive_density(path, matches, events_by_id):
    ...
    plot_page_frame(fig, "REFERENCE VISUAL | PROGRESSIVE PASS DENSITY", "Where Tromsø progression starts and finishes",
...
def save_switches_analysis(path, matches, events_by_id):
    ...
    plot_page_frame(fig,"REFERENCE VISUAL | SWITCHES OF PLAY","Switch frequency, spatial distribution and top contributors","season league ranking and Tromsø switch map")
    fig.savefig(path,facecolor=NAVY,dpi=100);plt.close(fig)
...
def save_cutback_zones(path, matches, events_by_id):
    ...
    plot_page_frame(fig,"REFERENCE VISUAL | CUTBACK RECEPTION ZONES","Where Tromsø cutbacks arrive","season successful and unsuccessful cutback end locations")
    fig.savefig(path,facecolor=NAVY,dpi=100);plt.close(fig)
...
def save_zone_reception_rankings(path, matches, events_by_id):
    ...
    plot_page_frame(fig,"REFERENCE VISUAL | HALF-SPACE RECEPTION RANKINGS","League context for middle- and final-third half-space access","receptions per match | Tromsø highlighted")
    fig.savefig(path,facecolor=NAVY,dpi=100);plt.close(fig)
...
def save_verticality(path, matches, events_by_id):
    ...
    plot_page_frame(fig,"REFERENCE VISUAL | TEAM VERTICALITY","Average forward distance per completed pass","2026 Eliteserien league ranking")
    fig.savefig(path,facecolor=NAVY,dpi=100);plt.close(fig)
...
def save_finishing_scatter(path, matches, events_by_id):
    ...
    plot_page_frame(fig,"REFERENCE VISUAL | FINISHING EFFICIENCY","Non-penalty conversion versus xG per shot","2026 Eliteserien league comparison")
    fig.savefig(path,facecolor=NAVY,dpi=100);plt.close(fig)
...
def save_probability(path, xgf, xga):
    img = base_image()
    d = ImageDraw.Draw(img)
    header(d, "SHOOTING | SCORELINE MODEL", "xG scoreline probability matrix", f"Neutral opposition-only baseline: Tromsø λ={xgf:.2f}, opponent λ={xga:.2f}")
    ...
    d.text((left - 140, top + cell * 3), "TROMSØ GOALS", fill=MUTED, font=font(20, True), anchor="mm")
    ...
    outcomes = [("TROMSØ WIN", home_win, CYAN), ("DRAW", draw, AMBER), ("OPPONENT WIN", away_win, PINK)]
...
def save_scatter(path, stats):
    img = base_image()
    d = ImageDraw.Draw(img)
    header(d, "SHOOTING | LEAGUE CONTEXT", "Shots per 90 vs xG per 90", "2026 Eliteserien - team attacking output")
    ...
    for t, v in stats.items():
        ...
        label = "TROMSØ" if focus else t.replace("HNK ", "").replace("Lokomotiva Zagreb", "Lokomotiva")
...
def save_xg_type(path, season_shots):
    img = base_image()
    d = ImageDraw.Draw(img)
    header(d, "SHOOTING | CHANCE SOURCE", "xG by chance type", "Tromsø season totals; share of total expected goals")
...
def create_assets(matches, all_shots, stats, events_by_id):
    ...
    save_shotmap(
        ASSET_DIR / "01_tromso_season_shotmap.png",
        season_shots,
        "Tromsø season shot map",
        f"2026 Eliteserien | {len(team_matches)} matches | attacking direction bottom to top"
    )
    recent_paths = []
    for i, m in enumerate(recent, 1):
        shots = [s for s in all_shots if s["match_id"] == m["match_id"] and s["team"] == TEAM]
        venue = "HOME" if team_name(m, "home") == TEAM else "AWAY"
        p = ASSET_DIR / f"0{i+1}_tromso_last3_{m['match_date']}_{safe_filename(opponent(m))}.png"
        save_shotmap(p, shots, f"Tromsø {result_string(m)} {opponent(m)}", f"{m['match_date']} | {venue} | matchweek {m['match_week']}")
        recent_paths.append(p)
    flow_paths = []
    for i, m in enumerate(recent, 1):
        p = ASSET_DIR / f"0{i+4}_tromso_xg_flow_{m['match_date']}_{safe_filename(opponent(m))}.png"
    ...
    for i, m in enumerate(recent, 1):
        safe_opp = safe_filename(opponent(m))
    ...
    prob = ASSET_DIR / "17_tromso_xg_scoreline_matrix.png"
    scatter = ASSET_DIR / "18_hnl_shots90_vs_xg90.png"
    types = ASSET_DIR / "19_tromso_xg_by_type.png"
    ...
    return {
        "season": ASSET_DIR / "01_tromso_season_shotmap.png",
        "recent": recent_paths,
...
def pdf_cover(c, width, height, stats, recent):
    ...
    c.drawString(52, height-145, "TROMSØ IL")
    ...
    c.setFillColor(MUTED)
    c.setFont("Arial", 10)
    c.drawRightString(width-42, 32, "2026 Eliteserien | StatsBomb event data")
...
def pdf_contents(c, width, height):
    ...
    sections = [
        ...
        ("09", "HOW TO HURT TROMSØ", "94-103", PINK),
    ]
...
def create_pdf(assets, stats):
    ...
    c.setTitle("Tromsø IL Complete Opposition Report")
...
def write_summary(matches, assets, stats):
    ...
    text = f"""TROMSØ IL COMPLETE OPPOSITION REPORT

Season: 2026 Eliteserien
Matches: {v['games']}
...
Method note:
The scoreline matrix uses independent Poisson distributions with Tromsø season xG-for and xG-against per match. It is an opposition-only neutral baseline, not a Hradec-specific forecast.
The shooting index is 70% league-normalized xG per match and 30% league-normalized shots per match.
"""
    (OUT_DIR / "Tromso_IL_opposition_report_notes.txt").write_text(text)
